...
if ( ! function_exists( 'classicmag_ajax_pagination' ) ) :
    /**
     ...
     */
    function classicmag_ajax_pagination($type) {
        global $wp_query; 
        if ( $wp_query->max_num_pages > 1  ) {

            ?>
            <div class="classicmag-load-posts-btn-wrapper"
            data-page="1" 
            data-max-pages="<?php echo esc_attr($wp_query->max_num_pages);?>" 
            data-load-type="<?php echo esc_attr($type);?>"
            >
                <a href="#" class="">
                    <span class="classicmag-ajax-loader"></span>
                    <button class="load-btn"><?php _e('Load More', 'classicmag') ?></button>
                </a>
            </div>
            <?php
        }
...
if ( ! function_exists( 'classicmag_load_posts' ) ) :
    /**
     ...
     *
     */
    function classicmag_load_posts() {

        check_ajax_referer( 'classicmag-load-posts-nonce', 'nonce' );
        ...
        if($posts->have_posts()):
            ...
            $error = new WP_Error( '500', __('No More Posts','classicmag') );
...
add_action( 'wp_ajax_classicmag_load_posts', 'classicmag_load_posts' );
add_action( 'wp_ajax_nopriv_classicmag_load_posts', 'classicmag_load_posts' );