...
/**
 * Adds Classicmag_Author_Info widget.
 */
class Classicmag_Author_Info extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'classicmag_author_info_widget',
            esc_html__( 'Classicmag: Author Info', 'classicmag' ),
            array( 'description' => esc_html__( 'Displays author short info.', 'classicmag' ), )
        );

        $this->social_networks = apply_filters( 'classicmag_author_widget_social_networks', array(
                'facebook', 
                'twitter',
                'linkedin',
                'instagram',
    ...
    public function widget( $args, $instance ) {
        ...
        do_action( 'classicmag_before_author_info');
        ?>
                    ...
                    if(!empty($social_networks) && is_array($social_networks)){
                        foreach($social_networks as $network){
                            if(!empty($instance[$network])){
                                $svg = Classicmag_SVG_Icons::get_theme_svg_name( $instance[$network] );
                                if($svg){
                                    ?>
        ...
        do_action( 'classicmag_after_author_info');
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>">
                <?php esc_attr_e('Title:', 'classicmag'); ?>
            </label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('author_name')); ?>">
                <?php esc_attr_e('Author Name:', 'classicmag'); ?>
            </label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('author_desc')); ?>">
                <?php esc_attr_e('Short Description:', 'classicmag'); ?>
            </label>
        ...
        <div>
            <label for="<?php echo esc_attr( $this->get_field_id( 'author_img' ) ); ?>">
                <?php esc_attr_e('Author Image:', 'classicmag'); ?>
            </label>
            <?php
            ...
            <div class="image-field">
                ...
                <p>
                    ...
                    <button type="button" class="upload_image_button button" data-uploader-title-txt="<?php esc_attr_e( 'Use Image', 'classicmag' ); ?>" data-uploader-btn-txt="<?php esc_attr_e( 'Choose an Image', 'classicmag' ); ?>">
                        <?php _e( 'Upload/Add image', 'classicmag' ); ?>
                    </button>
                    <button type="button" class="remove_image_button button" style="<?php echo esc_attr($remove_button_style);?>"><?php _e( 'Remove image', 'classicmag' ); ?></button>
                </p>
            </div>
        </div>
        <div>
            <p>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('image_border_radius')); ?>">
                    <?php _e('Enable Imaged Border Radius', 'classicmag'); ?>
                </label>
            </p>
        </div>
        <div>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'style' ) ); ?>">
                    <span class="field-label"><?php _e('Style', 'classicmag'); ?></span>
                </label>
                <select id="<?php echo esc_attr( $this->get_field_id( 'style' ) ); ?>" name="<?php echo esc_attr($this->get_field_name('style')); ?>" class="widefat">
                    <option value="style_1" <?php selected('style_1', $style);?>><?php _e('Style 1', 'classicmag'); ?></option>
                    <option value="style_2" <?php selected('style_2', $style);?>><?php _e('Style 2', 'classicmag'); ?></option>
                    <option value="style_3" <?php selected('style_3', $style);?>><?php _e('Style 3', 'classicmag'); ?></option>
                </select>
            </p>
        </div>
        <?php
        ...
        if(!empty($social_networks) && is_array($social_networks)){
            foreach ($social_networks as $network){
                ...
                ?>
                <p class="classicmag-social-link">
