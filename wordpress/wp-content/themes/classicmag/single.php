...
if ($classicmag_post_layout == "layout-2") { ?>
    <div class="single-featured-banner">
        <div class="featured-banner-media">
            <div class="data-bg image-size-large" data-background="<?php echo esc_url($featured_image); ?>"></div>
        </div>
        <div class="featured-banner-content">
            <div class="wrapper">
                ...
                <header class="entry-header">
                    <?php
                    if ( is_singular() ) :
                        the_title( '<h1 class="entry-title font-size-large mb-24">', '</h1>' );
                    else :
        ...
        <div class="wrapper">
            <div id="primary" class="content-area theme-sticky-component">
                ...
                while (have_posts()) :
                    ...
                    the_post_navigation(
                        array(
                            'prev_text' => '<span class="nav-subtitle">' . esc_html__('Previous:', 'classicmag') . '</span> <span class="nav-title">%title</span>',
                            'next_text' => '<span class="nav-subtitle">' . esc_html__('Next:', 'classicmag') . '</span> <span class="nav-title">%title</span>',
                        )
                    );
                    ...
                    if ('post' === get_post_type()) :
                        ...
                        $show_author_info = classicmag_get_option('show_author_info');
                        $show_related_posts = classicmag_get_option('show_related_posts');
                        $show_author_posts = classicmag_get_option('show_author_posts');
            ...
            <?php
            $page_layout = classicmag_get_page_layout();
            if (!empty($page_layout) && 'no-sidebar' != $page_layout):
                get_sidebar();
            endif;
            ?>
        </div>
...
<?php $show_sticky_article_navigation = classicmag_get_option('show_sticky_article_navigation');
    ...
    <div class="sticky-article-navigation">
        <?php if( isset( $prev_post->ID ) ){
            ...
            <a class="sticky-article-link sticky-article-prev" href="<?php echo esc_url( $prev_link ); ?>">
                <div class="sticky-article-icon">
                    <?php classicmag_theme_svg('arrow-left'); ?>
                </div>
        ...
        if( isset( $next_post->ID ) ){
            ...
            <a class="sticky-article-link sticky-article-next" href="<?php echo esc_url( $next_link ); ?>">
                <div class="sticky-article-icon">
                    <?php classicmag_theme_svg('arrow-right'); ?>
                </div>
                <article id="post-<?php the_ID(); ?>" <?php post_class('theme-article-post theme-sticky-article'); ?>>
                    <?php if (has_post_thumbnail()): ?>
                        <div class="entry-image">
